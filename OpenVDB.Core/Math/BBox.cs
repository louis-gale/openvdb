...
using System;
using System.Numerics; // For IFloatingPointIeee754, INumber
...
namespace OpenVDB.Math
{
        ...
        where TElement : struct, INumber<TElement>, IMinMaxValue<TElement>
    {
        ...
        private static class PointOps
        {
            ...
            {
                if (p is Vec3<TElement> v) return v.X;
                if (p is Coord c && typeof(TElement) == typeof(int)) return (TElement)(object)c.X;
                throw new NotSupportedException("GetX not supported for TPoint type.");
            }
            public static TElement GetY(TPoint p)
            {
                if (p is Vec3<TElement> v) return v.Y;
                if (p is Coord c && typeof(TElement) == typeof(int)) return (TElement)(object)c.Y;
                throw new NotSupportedException("GetY not supported for TPoint type.");
            }
            public static TElement GetZ(TPoint p)
            {
                if (p is Vec3<TElement> v) return v.Z;
                if (p is Coord c && typeof(TElement) == typeof(int)) return (TElement)(object)c.Z;
                throw new NotSupportedException("GetZ not supported for TPoint type.");
            }

            public static TPoint Add(TPoint p, TPoint q)
            {
                if (p is Vec3<TElement> pv && q is Vec3<TElement> qv) return (TPoint)(object)(pv + qv);
                if (p is Coord pc && q is Coord qc && typeof(TElement) == typeof(int)) return (TPoint)(object)(pc + qc);
                throw new NotSupportedException("Add not supported.");
            }
            public static TPoint Add(TPoint p, TElement val)
            {
                if (p is Vec3<TElement> pv) return (TPoint)(object)(pv + val);
                if (p is Coord pc && typeof(TElement) == typeof(int)) return (TPoint)(object)(pc + (int)(object)val); // Coord + int
        ...
        public BBox(TPoint minDefault, TPoint maxDefault, bool isEmpty = true) // Internal helper constructor
        {
            if (isEmpty)
            {
                Min = PointOps.Create(TElement.MaxValue);
                Max = PointOps.Create(TElement.MinValue);

                 // For Vec3<float/double>, MaxValue/MinValue are positive/negative infinity or Max/Min representable.
                 if (typeof(TElement) == typeof(float))
                 {
                    Min = (TPoint)(object)new Vec3<float>(float.MaxValue, float.MaxValue, float.MaxValue);
                    Max = (TPoint)(object)new Vec3<float>(float.MinValue, float.MinValue, float.MinValue);
                 } else if (typeof(TElement) == typeof(double)) {
                    Min = (TPoint)(object)new Vec3<double>(double.MaxValue, double.MaxValue, double.MaxValue);
                    Max = (TPoint)(object)new Vec3<double>(double.MinValue, double.MinValue, double.MinValue);
                 }
            }
            else
            {
        ...
        public BBox(TPoint min, TPoint max, bool sorted)
        {
            ...
            {
                Min = PointOps.MinComponents(min, max);
                Max = PointOps.MaxComponents(min, max);
            }
        }
        ...
        public static BBox<TPoint, TElement> CreateEmpty()
        {
             ...
             } else if (typeof(TPoint) == typeof(Coord)) { // For Coord (int elements)
        ...
        {
            if (IsEmpty) return PointOps.Create(TElement.Zero);
        ...
        {
            if (IsEmpty) return TElement.Zero;
            var ext = Extents();
        ...
        public void Expand(TElement padding)
        {
            ...
            } else { throw new NotSupportedException("Expand by padding not supported for TPoint.");}
        }
